...
/**
  ******************************************************************************
  * @file           : main.c
  * @author			: Cássio F. Braga, Nathan E. Nogara
...
//Sinal de saída do filtro em float
...
int main(void)
{
  ...
  MX_DAC_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start(&htim2); // Timer da frequência de amostragem
  HAL_ADC_Start_DMA(&hadc1, adc, NDMA); // Coleta da amostra do sinal de entrada
  HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_1, dac, NDMA, DAC_ALIGN_12B_R); // Emissão do resultado do filtro
  arm_fir_init_f32(&instancia, NUM_TAPS, (float32_t *)&coeficientes[0], (float32_t *)&firStateF32[0], blockSize); //Inicialização do filtro
  /* USER CODE END 2 */
...
/*
 * @brief Callback do fim da conversão do ADC
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    ...
    HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin); //Inicia a contagem do tempo de cálculo

    arm_fir_f32(&instancia, &signal_in, &signal_out, blockSize); //Filtro com os valores atuais

    HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin); //Termina a contagem do tempo de cálculo
